...
variable "diagnostic_settings" {
  ...
  default     = {}
  description = <<DESCRIPTION
A map of diagnostic settings to create on the ExpressRoute Port. The map key is deliberately arbitrary to avoid issues where map keys maybe unknown at plan time.

- `name` - (Optional) The name of the diagnostic setting. One will be generated if not set, however this will not be unique if you want to create multiple diagnostic setting resources.
- `log_categories` - (Optional) Diagnostic log categories. Included for AVM interface compatibility; this resource currently configures metrics only.
- `log_groups` - (Optional) Diagnostic log groups. Included for AVM interface compatibility; this resource currently configures metrics only.
- `metric_categories` - (Optional) A set of metric categories to send to the destination. Only `AllMetrics` is supported. Defaults to `["AllMetrics"]`.
- `log_analytics_destination_type` - (Optional) The destination type for the diagnostic setting. Possible values are `Dedicated` and `AzureDiagnostics`. Defaults to `Dedicated`.
- `workspace_resource_id` - (Optional) The resource ID of the log analytics workspace to send metrics to.
- `storage_account_resource_id` - (Optional) The resource ID of the storage account to send metrics to.
- `event_hub_authorization_rule_resource_id` - (Optional) The resource ID of the event hub authorization rule to send metrics to.
- `event_hub_name` - (Optional) The name of the event hub. If none is specified, the default event hub will be selected.
- `marketplace_partner_resource_id` - (Optional) The full ARM resource ID of the Marketplace resource to which you would like to send metrics.
DESCRIPTION
  nullable    = false
